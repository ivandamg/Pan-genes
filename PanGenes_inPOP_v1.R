...
# Script calculating and plotting Pan GENES
# IM 24.11.17
...
#################################################################################################################
#################################################################################################################
# HOMOLOGS of all samples in directory IN N16961
########################

# In bash find homologs of N16961 -> In order to compare
#a=0;for i in $(ls Vibrio_cholerae*.faa); do echo $(echo $i | cut -d'_' -f3) ;blastp -db db_prot_genomes/$(echo $i | cut -d'_' -f3)_db -outfmt 6 -evalue 1e-6 -show_gis -num_alignments 1 -max_hsps 20 -num_threads 30 -out Homologs_N16961/Homolog_N16961_$(echo $i | cut -d'_' -f3).xml -query ~/Documents/EPFL/Vibrio_Chromosomal_rearrangements/PopGenomics_V2/ALL_POPS/CoreGenes/Vibrio_cholerae_N16961_Prokka.faa ; done

# For each sample find EQUIVALENT ON N16961 -> have a common ID.
setwd('~/Documents/EPFL/Vibrio_Chromosomal_rearrangements/PopGenomics_V2/ALL_POPS/CoreGenes/Homologs_N16961/')
...
names(Homolog_info)<-gsub(".xml","",gsub("Homolog_N16961_","",filesToProcess))

# Create column header
colnam<-names(Homolog_info)

for (i in 1:length(filesToProcess)) {
  NAME<-c(unlist(colnam[[i]]),"Gene","Identity","Length","Mnn","Humm","Start_strain1","End_strain1","Start_strain2","End_strain2","Evalue","Bitscore")
...
#########################################################################
######### CALCULATION OF PAN GENES per POP ##############################
#########################################################################
...
### STRUCTURE INPUT FILE BASED ON PRESENCE ABSENCE OF GENES IN ALL POP
rownames(VariableGenes_Dist)

Populations<-read.csv("~/Documents/EPFL/Vibrio_Chromosomal_rearrangements/PopGenomics_V2/ALL_POPS/Phylogenies_ALL/Strains_Pop_vf.csv", h=T)
popColor<-data.frame(strain=rownames(VariableGenes_Dist))
popColor<-merge(Populations,popColor,by = "strain")

popColor$population<- as.character(popColor$population)
popColor[popColor=="Pandemic"]<- 1
popColor[popColor=="Mexico"]<- 2
popColor[popColor=="California"]<- 3
popColor[popColor=="Haiti"]<- 4
popColor[popColor=="Massachusetts"]<- 5
popColor[popColor=="Thailand"]<- 6
popColor<-popColor[match(rownames(VariableGenes_Dist), popColor$strain),]


Structure_vf<-cbind.data.frame(popColor$strain,popColor$population,VariableGenes_Dist)
write.table(Structure_vf,"~/Documents/EPFL/Vibrio_Chromosomal_rearrangements/PopGenomics_V2/ALL_POPS/BetweenPOPS_VAR/Structure/Variable_within_POP.txt", quote = F,row.names = F)
#################################################################
#########################################################################
